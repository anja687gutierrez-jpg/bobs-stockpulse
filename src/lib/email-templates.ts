import type { TechnicalSignal, CalendarEvent, GapAlert, MarketMover, TrendingStock, VolumeSpike, FiftyTwoWeekAlert, IndexSnapshot, PostEarningsMove } from "./types";
...
export function dailyMarketEmail(params: {
  ...
  fiftyTwoWeekAlerts: FiftyTwoWeekAlert[];
  indices: IndexSnapshot[];
  vix: number | null;
  postEarningsMoves: PostEarningsMove[];
  signals: TechnicalSignal[];
  events: CalendarEvent[];
  gainers: MarketMover[];
  losers: MarketMover[];
  ...
  const { holdings, significantMovers, totalValue, totalChange, gaps, volumeSpikes, fiftyTwoWeekAlerts, indices, vix, postEarningsMoves, signals, events, gainers, losers, trending } = params;
  const changeColor = totalChange >= 0 ? "#34d399" : "#f87171";
  const changeSign = totalChange >= 0 ? "+" : "";
  ...
  if (gaps.length > 0) {
    subjectParts.push(`${gaps.length} overnight gap${gaps.length !== 1 ? "s" : ""}`);
  }
  ...
  let marketContext = "";
  if (indices.length > 0 || vix !== null) {
    const indexCells = indices
      .map((idx) => {
        const color = idx.changePercent >= 0 ? "#34d399" : "#f87171";
        const sign = idx.changePercent >= 0 ? "+" : "";
        return `
          <td style="padding: 8px 12px; text-align: center; width: 25%;">
            <div style="font-weight: 600; color: #fbbf24; font-size: 13px;">${idx.ticker}</div>
            <div style="font-size: 16px; font-weight: bold;">$${idx.price.toFixed(2)}</div>
            <div style="color: ${color}; font-size: 13px; font-weight: 600;">${sign}${idx.changePercent.toFixed(2)}%</div>
          </td>`;
      })
      .join("");

    const vixCell = vix !== null
      ? `<td style="padding: 8px 12px; text-align: center; width: 25%;">
            <div style="font-weight: 600; color: #fbbf24; font-size: 13px;">VIX</div>
            <div style="font-size: 16px; font-weight: bold;">${vix.toFixed(2)}</div>
            <div style="color: ${vix >= 30 ? "#f87171" : vix >= 20 ? "#fbbf24" : "#34d399"}; font-size: 13px; font-weight: 600;">${vix >= 30 ? "High Fear" : vix >= 20 ? "Elevated" : "Calm"}</div>
          </td>`
      : "";

    marketContext = `
      <div style="background: #2c2c2e; border-radius: 8px; padding: 12px 4px; margin-bottom: 20px;">
        <table style="width: 100%; border-collapse: collapse;">
          <tr>${indexCells}${vixCell}</tr>
        </table>
      </div>`;
  }
  ...
  if (gaps.length > 0) {
    ...
    const gapRows = sortedGaps
      ...
      .join("");
    gapSection = `
      <h3 style="color: #f87171; margin: 20px 0 8px; font-size: 16px;">Overnight Gaps (${gaps.length})</h3>
      <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
        <thead>
          <tr style="color: #6b7280; border-bottom: 1px solid #374151;">
            <th style="padding: 6px 8px; text-align: left;">Ticker</th>
  ...
  if (volumeSpikes.length > 0) {
    const volumeRows = volumeSpikes
      .map(
        (v) =>
          `<tr>
            <td style="padding: 6px 8px; font-family: monospace; color: #fbbf24;">${v.ticker}</td>
            <td style="padding: 6px 8px; color: #9ca3af; font-size: 12px;">${v.name}</td>
            <td style="padding: 6px 8px; text-align: right;">${formatVolume(v.volume)}</td>
            <td style="padding: 6px 8px; text-align: right; color: #9ca3af;">${formatVolume(v.avgVolume)}</td>
            <td style="padding: 6px 8px; text-align: right; color: #a78bfa; font-weight: 600;">${v.ratio.toFixed(1)}x</td>
          </tr>`
      )
      .join("");
    volumeSection = `
      <h3 style="color: #a78bfa; margin: 20px 0 8px; font-size: 16px;">Volume Spikes (${volumeSpikes.length})</h3>
      <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
        <thead>
          <tr style="color: #6b7280; border-bottom: 1px solid #374151;">
            <th style="padding: 6px 8px; text-align: left;">Ticker</th>
            ...
            <th style="padding: 6px 8px; text-align: right;">Avg (10d)</th>
  ...
  let fiftyTwoWeekSection = "";
  if (fiftyTwoWeekAlerts.length > 0) {
    const alertRows = fiftyTwoWeekAlerts
      .map(
        (a) => {
          const isHigh = a.level === "high";
          const label = isHigh ? "Near High" : "Near Low";
          const color = isHigh ? "#34d399" : "#f87171";
          return `<tr>
            <td style="padding: 6px 8px; font-family: monospace; color: #fbbf24;">${a.ticker}</td>
            <td style="padding: 6px 8px; color: #9ca3af; font-size: 12px;">${a.name}</td>
            <td style="padding: 6px 8px; text-align: right;">$${a.price.toFixed(2)}</td>
            <td style="padding: 6px 8px; text-align: right;"><span style="color: ${color}; font-weight: 600;">${label}</span></td>
            <td style="padding: 6px 8px; text-align: right; color: #9ca3af;">${a.distance.toFixed(1)}% away</td>
          </tr>`;
        }
      )
      .join("");
    fiftyTwoWeekSection = `
      <h3 style="color: #fbbf24; margin: 20px 0 8px; font-size: 16px;">52-Week Watchlist (${fiftyTwoWeekAlerts.length})</h3>
      <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
        <thead>
          <tr style="color: #6b7280; border-bottom: 1px solid #374151;">
            <th style="padding: 6px 8px; text-align: left;">Ticker</th>
  ...
  // --- Post-Earnings Moves ---
  let postEarningsSection = "";
  if (postEarningsMoves.length > 0) {
    const earningsRows = postEarningsMoves
      .map(
        (p) =>
          `<tr>
            <td style="padding: 6px 8px; font-family: monospace; color: #fbbf24;">${p.ticker}</td>
            <td style="padding: 6px 8px; color: #9ca3af; font-size: 12px;">${p.name}</td>
            <td style="padding: 6px 8px; text-align: right;">$${p.price.toFixed(2)}</td>
            <td style="padding: 6px 8px; text-align: right; color: ${p.changePercent >= 0 ? "#34d399" : "#f87171"}; font-weight: 600;">${p.changePercent >= 0 ? "+" : ""}${p.changePercent.toFixed(2)}%</td>
          </tr>`
      )
      .join("");
    postEarningsSection = `
      <h3 style="color: #fbbf24; margin: 20px 0 8px; font-size: 16px;">Earnings Day Movers (${postEarningsMoves.length})</h3>
      <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
        <thead>
          <tr style="color: #6b7280; border-bottom: 1px solid #374151;">
            <th style="padding: 6px 8px; text-align: left;">Ticker</th>
            <th style="padding: 6px 8px; text-align: left;">Name</th>
            <th style="padding: 6px 8px; text-align: right;">Price</th>
            <th style="padding: 6px 8px; text-align: right;">Reaction</th>
          </tr>
        </thead>
